...
class GenInsertEnv(object):
    def __init__(self, c_obj):
        ...
        self.master_control = {
            'into': [self.into_observe, self.into_action],
        ...
        self.action_space = self.observation_space = len(self.c_obj.word_num_map)
        self.cur_state = self.master_control['into']  # 初始时为from
        self.time_step = 0
        self.bug_reward = c_obj.bug_reward

    def reset(self):
        self.cur_state = self.master_control['into']
        self.cur_table = ""
        self.into_clause = ""
        self.tuple_clause = ""
        self.where_clause = ""
        self.subquery_clause = ""
        self.sql = ""
        return self.c_obj.word_num_map['into']

    def get_sql(self):
        if self.into_clause == "":
            return ""
        if self.tuple_clause == "" and self.cur_table != "":
            self.tuple_clause = "select * from {}".format(self.c_obj.num_word_map[self.cur_table])
        self.sql = "insert into {} {}".format(self.into_clause, self.tuple_clause)
        if self.where_clause != "":
            self.sql = self.sql + " " + self.where_clause
        return self.sql

    def into_observe(self, observation=None):
        # print('enter into observation')
        # print(self.c_obj.num_word_map[observation])
        candidate_word = np.zeros((self.action_space,), dtype=int)
        if observation == self.c_obj.word_num_map['into']:
    ...
    def into_action(self, action=None):
        # print('enter into action')
        # print(self.c_obj.num_word_map[action])
        if action in self.c_obj.tables:
            self.cur_table = action
            self.into_clause = self.c_obj.num_word_map[action]
        elif action == self.c_obj.word_num_map['into']:
            pass
    ...
    def select_observe(self, observation=None):
        """
        :param observation:
        :return:
        """
        # print('enter into select')
        # print(self.c_obj.num_word_map[observation])
        candidate_word = np.zeros((self.action_space, ), dtype=int)
        if observation == self.c_obj.word_num_map['select']:
    ...
    def select_action(self, action=None):
        ...
        :return:
        """
        if action == self.c_obj.word_num_map['select']:
            self.tuple_clause = "select * from {}".format(self.c_obj.num_word_map[self.cur_table])
        elif action in self.c_obj.keyword:
            self.cur_state = self.master_control[self.c_obj.num_word_map[action]]
            self.cur_state[1](action)
    ...
    def observe(self, observation):
        if observation == self.c_obj.word_num_map['insert']:
            return self.cur_state[0](self.c_obj.word_num_map['into'])
    ...
    def step(self, action):
        ...
        elif action == self.c_obj.word_num_map['insert']:
            return self.cur_state[1](self.c_obj.word_num_map['into'])
        else:
