...
SEQ_LENGTH = 20     # sequence lengthå
SEED = 88
BATCH_SIZE = 32
...
class AC_Agent(keras.Model):
    def __init__(self, env):
        ...
        self.episode = 0
        self.max_episodes = BATCH_SIZE * 10
        ...
        if self.ac_log:
            ac_path = tpath + '_ac'
            self.a_log = open(ac_path, 'w')
        if self.time_log:
            print('cal_time...')
            print('predata', self.pre_memory.shape[0])
            time.sleep(5)
            self.sqls = list()
            sql_path = tpath + '_sql'
            t_path = tpath + '_time'
    ...
    def load_predata(self):
        ...
        # tpath = cpath + '/' + self.env.dbname + '/' + 'cost_rc10000_11000_predata.npy'
            ...
            while self.episode < self.max_episodes or (self.time_log and s_count < 1000):   # 前面算ac，后面算time
                ...
                if ep_steps == SEQ_LENGTH or reward == self.env.bug_reward:
                    ...
                    if reward == -2:
                        ...
                        self.end_of_episode_actions(reward, ep_steps)
                    if self.time_log:
                        if reward >= self.env.target_type:
                            sql = self.env.get_sql()
                            self.sqls.append(sql)
                            print(sql, "reward:", reward)
                            s_count += 1
                            if s_count % 10 == 0:
