...
fmetric = open('/home/jintao/naru_update/metric_result/' + 'imdb' + '.update_neuro.txt', 'a')
...
class NeuroCard(tune.Trainable):
    ...
    def _setup(self, config):
        ...
        if self.dataset == 'imdb' and len(self.join_tables) > 1:
            ...
            self.loaded_queries, self.oracle_cards = utils.UnpackQueries(
                self.table, queries_job_format)  # 瑙ｆ瀽杩囩▼锛岄渶瑕佹浛鎹?        timepre1 = time.time()
    ...
    def _train(self):
        if self.checkpoint_to_load and args.update == 0:  # or self.eval_join_sampling:
    ...
    def _save(self, tmp_checkpoint_dir):
        if (self.checkpoint_to_load and args.update == 0) or not self.save_checkpoint_at_end:
            return {}
    ...
    def Query(self,
        ...
        for est in estimators:
            ...
            if (estimators[0] == est):
                met0.append(card)
                mee0.append(est_card)  # 鍙互涓や釜浼拌鍣?            if(len(estimators)==2):
                if (estimators[1] == est):
                    met1.append(card)  # 瑕佸啀娆″垽鏂紝鍏稿瀷娌℃湁鎬濊€冩竻妤氶€昏緫鐨凚UG
