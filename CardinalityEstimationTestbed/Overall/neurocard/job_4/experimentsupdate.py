...
# JOB-light-ranges, NeuroCard base.     # 鏀硅繖閲岋紒锛侊紒
'job-light-ranges': dict(
    dict(dict(BASE_CONFIG, **JOB_LIGHT_BASE), **FACTORIZE),
    **{
        'queries_csv': './queries/imdbtest.sql.csv',
        'use_cols': 'content',
...
# 淇敼杩欓噷
TEST_CONFIGS['job-light-ranges-update0'] = dict(
EXPERIMENT_CONFIGS['job-light-ranges'],
** {
    'checkpoint_to_load': 'models/imdb_before.pt',
    'eval_psamples': [512, 8000],
    ...
    'asserts': {
        'fact_psample_512_median': 2000000.0,
        'fact_psample_512_p99': 40000000,
        'fact_psample_8000_median': 10000000.9,
        'fact_psample_8000_p99': 40000000,
    },
})

TEST_CONFIGS['job-light-ranges-update1'] = dict(
EXPERIMENT_CONFIGS['job-light-ranges'],
** {
    'checkpoint_to_load': 'models/imdb_before.pt'
    # Evaluating on all queries takes a while.  Shorten the wait by
