                ...
                for t in full_tables:
                    ...
                    if t != 'auth_user':
                        datas.append(pd.read_csv('{}/{}.csv'.format(datasets_dir, t), quotechar='"', escapechar='\\',
                            ...
                            ['user_id']).size().reset_index(name='counts').set_index('user_id'))
                total = 0
                for id in tqdm(pd.read_csv('{}/auth_user.csv'.format(datasets_dir))['id'].tolist()):
                    num = 1
                    for d in datas:
                        if id in d.index:
                            num *= d.loc[id, 'counts']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output-dir', type=str, help='output relative directory', default='JOB/cols-sql/4/test')
    ...
    for k, v in pattern2training.items():
        ...
        np.savetxt(
            '/home/jintao/CardinalityEstimationBenchmark/quicksel/test/java/edu/illinois/quicksel/resources/{}/{}.assertion'.format(
                args.output_dir, k), data, delimiter=",")
        vecs = []
        ...
        vecs.append(1.0)
        np.savetxt(
            '/home/jintao/CardinalityEstimationBenchmark/quicksel/test/java/edu/illinois/quicksel/resources/{}/{}.permanent'.format(
                args.output_dir, k), np.array([vecs]), delimiter=",")
