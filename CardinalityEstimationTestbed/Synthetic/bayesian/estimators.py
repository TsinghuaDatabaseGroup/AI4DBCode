...
class CardEst(object):
    ...
    def Query(self, columns, operators, vals, col=None):
    ...
    def AddError(self, err, est_card, true_card, est_avg, true_avg):
        self.errs.append(err)
        self.est_cards.append(est_card)
        self.true_cards.append(true_card)
        self.est_avgs.append(est_avg)
        self.true_avgs.append(true_avg)
    ...
    def merge_stats(self, state):
        ...
        self.est_avgs.extend(state[5])
        self.true_avgs.extend(state[6])
...
class ProgressiveSampling(CardEst):
    ...
    def _sample_n(self,
                  ...
                  col=None):
        ncols = len(columns)
        avg_logits = logits = self.init_logits
        if inp is None:
        ...
        for i in range(ncols):
            ...
            if op is not None:
                ...
                temp_a = columns[natural_idx].all_distinct_values
                temp_b = vals[natural_idx]
        ...
        for ls in masked_probs[2:]:
            p *= ls
        p *= masked_probs[0]
        return p.mean().item(), ((p * sample_result).sum() / p.sum()).item() if sample_result is not None else np.nan
    ...
    def Query(self, columns, operators, vals, col=None):
        ...
        with torch.no_grad():
            ...
            return np.ceil(np.mean(ps) * self.cardinality).astype(
                dtype=np.int32, copy=False), np.sum(results) / np.sum(ps) if col is not None else None
...
class Oracle(CardEst):
    ...
    def Query(self, columns, operators, vals, return_masks=False, col=None):
