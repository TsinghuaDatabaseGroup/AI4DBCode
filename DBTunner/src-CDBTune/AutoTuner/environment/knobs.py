...
def init_knobs(instance,num_total_knobs):
    global memory_size
    global disk_size
    global KNOB_DETAILS
...
def save_knobs(knob, metrics, instance,task_id):
    ...
    # sdb.update("insert into tb_task_result(task_id,knobs_detail,tps,rt,qps,score) values(%d,'%s',%f,%f,%f,%f)" % (task_id,knob_json,metrics[0], metrics[1], metrics[2],metrics[2]/metrics[1]))
    
    
    
    data = dict()
    data["task_id"] = task_id
    data["knobs_detail"] = knob_json
    data["tps"] = metrics[0]
    data["rt"] = metrics[1]
    data["qps"] = metrics[2]
